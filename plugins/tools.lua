--Begin Tools.lua :)
local SUDO = 270541145 -- put Your ID here! <===
...
local text = matches[2]
tdcli.createNewChannelChat(text, 1, '@TelePlus', (function(b, d) tdcli.addChatMember(d.id_, msg.from.id, 0, dl_cb, nil) end), nil)
   if not lang then 
...
if ((matches[1] == 'start' and not Clang) or (matches[1] == "استارت" and Clang)) then
...
_Sudoer And Admins TelePlus Bot Help :_
...
text = [[
_راهنمای ادمین و سودو های ربات تله پلاس:_
